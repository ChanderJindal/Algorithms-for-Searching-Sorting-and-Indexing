...
class MaxHeap:
	...
	def maxHeapify(self, pos):
		...
		# than any of its child
		if not self.isLeaf(pos): # non-leaf 
			if (self.Heap[pos] < self.Heap[self.leftChild(pos)] or self.Heap[pos] < self.Heap[self.rightChild(pos)]): #it has either a left or right child greater than the parent so, swap is needed


				if (self.Heap[self.leftChild(pos)] > self.Heap[self.rightChild(pos)]):#left is greater 
					self.swap(pos, self.leftChild(pos)) #swap left 
					self.maxHeapify(self.leftChild(pos)) # check the rest
				else: # right is greater 
					self.swap(pos, self.rightChild(pos)) # swap right
					self.maxHeapify(self.rightChild(pos)) #check rest
	...
	def Pop(self): # finding the top
		...
		self.Heap[self.FRONT] = self.Heap[self.size] #replacing root with lowest order element
		self.size -= 1
		self.maxHeapify(self.FRONT) # remaking heap with the root // Bubble down so to say
...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        if element > minheap.Heap[minheap.FRONT] or minheap.size <= maxheap.size:
            maxheap.insert(minheap.pop())
            minheap.insert(element)
        else:
            minheap.insert(element)
        if minheap.size > maxheap.size:
            element = minheap.Heap[minheap.FRONT]
        else:
            element = maxheap.Heap[minheap.FRONT]
        print("The current middle element is ",element )
